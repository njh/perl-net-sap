/*
 *  mcast_socket: Multicast IPv6 and IPv4 socket code
 *
...
_leave_group( mcast_socket_t* sock )
{
	...
	switch (sock->saddr.ss_family) {
		case AF_INET: {
			if (setsockopt(sock->fd_recv, IPPROTO_IP, IP_DROP_MEMBERSHIP,
						   &(sock->imr), sizeof(sock->imr))==-1) {
		...
		case AF_INET6: {
			if (setsockopt(sock->fd_recv, IPPROTO_IPV6, IPV6_DROP_MEMBERSHIP,
						   &(sock->imr6), sizeof(sock->imr6))==-1) {
